...
class ResetPassword extends React.Component {
  ...
  login = (password) => {
    ...
    if (email && password) {
      postData(`/api/login`, {email, password}).then(() => {
        this.toMainPage()
      })
    }
  }
  ...
  confirm = () => {
    ...
    if (newPassword && passwordConfirmation) {
      if (newPassword == passwordConfirmation) {
          postData('/api/reset', {newPassword}).then(result => {
              if (result.success) {
                this.setState({ 
                    errMsg: "Пароль успешно изменён! Через пару секунд вы попадёте в свой аккаунт.",
                    errRed: [],
                }, () => {
                ...
                this.setState({
                  errMsg: "Возникла ошибка, возможно ссылка устарела",
  ...
  toMainPage = () => {
    window.location = "https://accounts.satoshkin.com/"
  }
            ...
            <tr className="title">
              <td className="titleTd">
                <div className="justLine">
                  <div style={{textAlign: "center", fontWeight: "900"}}>Сбросить пароль</div>
                </div>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td>
                <div className="justLine"> 
                  ...
                  <button style={{float: "right"}} className="bodyButton" onClick={this.toMainPage}>Отмена</button>
                </div>
              </td>
            </tr>
            <tr>
